...
from pyquokka.types import IQuokkaContext, NodeType, StageId, TaskGraphNodeId, TaskGraphNodeType
...
class TaskGraph:
    ...
    def __init__(self, context: IQuokkaContext) -> None:
        ...
        self.r = redis.Redis(
            host=str(self.context.cluster.leader_public_ip), port=6800, db=0)
        while True:
            try:
                _ = self.r.keys()
                break
    ...
    def epilogue(self, stage: StageId, placement_strategy):
    ...
    def new_input_dataset_node(self, dataset, stage=0):
        ...
        # print(channel_info)
        self.input_partitions[self.current_actor] = sum(
            [len(objects_dict[k]) for k in objects_dict])
        ...
        for ip in objects_dict:
            ...
            nodes = [
                k for k in self.context.io_nodes if self.context.node_locs[k] == ip]
            objects_per_node = (len(objects_dict[ip]) - 1) // len(nodes) + 1
            for i in range(len(nodes)):
                node = nodes[i]
                objects = list(range(
                    i * objects_per_node, min((i + 1) * objects_per_node, len(objects_dict[ip]))))
                if len(objects) == 0:
                    break
                # print(objects)
                lineages = [(ip, i) for i in objects]
                vals = {pickle.dumps((self.current_actor, count, seq)): pickle.dumps(
                    lineages[seq]) for seq in range(len(lineages))}
                input_task = TapedInputTask(self.current_actor, count, [
                                            i for i in range(len(lineages))])
                self.LIT.set(pipe, pickle.dumps(
                    (self.current_actor, count)), len(lineages) - 1)
        ...
        pipe.execute()
        ray.get(self.context.coordinator.register_actor_location.remote(
            self.current_actor, channel_locs))

        return self.epilogue(stage, DatasetStrategy(len(channel_locs)))

    def new_input_reader_node(self, reader, stage: StageId = 0, placement_strategy=None):
        ...
        assert type(placement_strategy) in [
            SingleChannelStrategy, CustomChannelsStrategy, TaggedCustomChannelsStrategy]

        channel_info = reader.get_own_state(
            self.get_total_channels_from_placement_strategy(placement_strategy, 'input'))
        # print(channel_info)
        self.input_partitions[self.current_actor] = sum(
            [len(channel_info[k]) for k in channel_info])
        ...
        if type(placement_strategy) == SingleChannelStrategy:
            ...
            nodes = sorted(self.context.io_nodes) if type(placement_strategy) == CustomChannelsStrategy else sorted(
                self.context.tag_io_nodes[placement_strategy.tag])
            ...
            for node in nodes:
                for channel in range(placement_strategy.channels_per_node):
                    ...
                    if len(lineages) > 0:
                        ...
                        self.DST.set(pipe, pickle.dumps(
                            (self.current_actor, count)), -1)
    ...
    def get_default_partition(self, source_node_id, target_placement_strategy):
        ...
        def partition_key_1(ratio, data, source_channel, num_target_channels):
            ...
            target_channel = int(random.random() * ratio) + \
                source_channel * ratio
            return {target_channel: data}
        ...
        if type(source_placement_strategy) == CustomChannelsStrategy or type(source_placement_strategy) == TaggedCustomChannelsStrategy:
            source_total_channels = self.get_total_channels_from_placement_strategy(
                source_placement_strategy, self.actor_types[source_node_id])
        ...
        if type(source_placement_strategy) == CustomChannelsStrategy or type(source_placement_strategy) == TaggedCustomChannelsStrategy:
            target_total_channels = self.get_total_channels_from_placement_strategy(
                target_placement_strategy, "exec")
    ...
    def prologue(self, streams, placement_strategy, source_target_info):
        ...
        def partition_key_str(key, data, source_channel, num_target_channels):
            ...
            if "int" in str(data[key].dtype).lower():
                partitions = data.with_columns(polars.Series(name="__partition__", values=(
                    data[key] % num_target_channels))).partition_by("__partition__")
                ...
                partitions = data.with_columns(polars.Series(name="__partition__", values=(
                    data[key].hash() % num_target_channels))).partition_by("__partition__")
            else:
                print(data[key])
        ...
        def partition_key_range(key, total_range, data, source_channel, num_target_channels):
            ...
            assert type(data) == polars.DataFrame
            partitions = data.with_columns(polars.Series(name="__partition__", values=(
                (data[key] - 1) // per_channel_range))).partition_by("__partition__")
            for partition in partitions:
                target = partition["__partition__"][0]
                result[target] = partition.drop("__partition__")
            return result
        ...
        for key in streams:
            ...
            if type(target_info.partitioner) == HashPartitioner:
                ...
                target_info.partitioner = partial(
                    partition_key_str, target_info.partitioner.key)
            elif type(target_info.partitioner) == RangePartitioner:
                target_info.partitioner = partial(
                    partition_key_range, target_info.partitioner.key, target_info.partitioner.total_range)
                ...
                source = streams[key]
                target_info.partitioner = self.get_default_partition(
                    source, placement_strategy)
            else:
                raise Exception("Partitioner not supported")

            target_info.lowered = True
            self.PFT.set(self.r, pickle.dumps(
                (source, self.current_actor)), ray.cloudpickle.dumps(target_info))

            # TODO: figure out why you need to do this to make it work
            self.PFT.get(self.r, pickle.dumps((source, self.current_actor)))

        registered = ray.get([node.register_partition_function.remote(sources, self.current_actor, self.get_total_channels_from_placement_strategy(
            placement_strategy, 'exec'), mapping) for node in (self.context.task_managers.values())])
        ...
        for stage in sorted(stage_sources.keys()):
            ...
            for source in stage_sources[stage]:
                num_channels = self.get_total_channels_from_placement_strategy(
                    self.actor_placement_strategy[source], self.actor_types[source])
            ...
            input_reqs.append(polars.from_dict(
                {"source_actor_id": source_actor_ids, "source_channel_id": source_channel_ids, "min_seq": min_seqs}))
    ...
    def new_non_blocking_node(self, streams, functionObject, stage=0, placement_strategy=CustomChannelsStrategy(1), source_target_info={}, assume_sorted={}):
        ...
        self.FOT.set(pipe, self.current_actor,
                     ray.cloudpickle.dumps(functionObject))

        start = time.time()
        input_reqs = self.prologue(
            streams, placement_strategy, source_target_info)
        ...
        for key in assume_sorted:
            ...
            sorted_source_channels = len(polars.concat(
                input_reqs).filter(polars.col("source_actor_id") == key))
        ...
        if type(placement_strategy) == SingleChannelStrategy:
            ...
            self.CLT.set(pipe, pickle.dumps(
                (self.current_actor, 0)), self.context.node_locs[node])
            self.IRT.set(pipe, pickle.dumps(
                (self.current_actor, 0, -1)), pickle.dumps(input_reqs))

        elif type(placement_strategy) == CustomChannelsStrategy or type(placement_strategy) == TaggedCustomChannelsStrategy:

            nodes = sorted(self.context.compute_nodes) if type(placement_strategy) == CustomChannelsStrategy else sorted(
                self.context.tag_compute_nodes[placement_strategy.tag])
            count = 0
            for node in nodes:
                for channel in range(placement_strategy.channels_per_node):
                    exec_task = ExecutorTask(
                        self.current_actor, count, 0, 0, input_reqs)
                    channel_locs[count] = node
                    self.NTT.rpush(pipe, node, exec_task.reduce())
                    self.CLT.set(pipe, pickle.dumps(
                        (self.current_actor, count)), self.context.node_locs[node])
                    self.IRT.set(pipe, pickle.dumps(
                        (self.current_actor, count, -1)), pickle.dumps(input_reqs))
                    count += 1
    ...
    def new_blocking_node(self, streams, functionObject, stage=0, placement_strategy=CustomChannelsStrategy(1), source_target_info={}, transform_fn=None, assume_sorted={}):
        ...
        current_actor = self.new_non_blocking_node(
            streams, functionObject, stage, placement_strategy, source_target_info, assume_sorted)
        self.actor_types[current_actor] = 'exec'
        dataset_id = ray.get(
            self.context.dataset_manager.create_dataset.remote())

        registered = ray.get([node.register_blocking.remote(current_actor, transform_fn,
                             self.context.dataset_manager, dataset_id) for node in list(self.context.task_managers.values())])
        assert all(registered)
        return dataset_id

    def create(self):

        initted = ray.get([node.init.remote()
                          for node in list(self.context.task_managers.values())])
        assert all(initted)
        ...
        topological_order = [
            k for k in topological_order if self.actor_types[k] != 'input']
        # print("topological_order", topological_order)
        ray.get(self.context.coordinator.register_actor_topo.remote(
            topological_order))
    ...
    def run(self):
        ...
        input_partitions = {}
        pbars = {k: tqdm(total=self.input_partitions[k])
                 for k in self.input_partitions}
        curr_source = 0
        for k in pbars:
            pbars[k].set_description(
                "Processing input source " + str(curr_source))

...
class TaskGraph:
    ...
    def __init__(self, context: IQuokkaContext) -> None:
        ...
        self.actor_placement_strategy: dict[TaskGraphNodeId, PlacementStrategy] = {}
    ...
    def get_total_channels_from_placement_strategy(
        self, placement_strategy: PlacementStrategy, node_type: NodeType
    ) -> NumTotalChannels:
        ...
        if type(placement_strategy) == SingleChannelStrategy:
            ...
            return (
                self.context.cluster.num_node
                * placement_strategy.channels_per_node
                * (
                    self.context.io_per_node
                    if node_type == "input"
                    else self.context.exec_per_node
                )
            )
        elif type(placement_strategy) == TaggedCustomChannelsStrategy:
            return (
                len(self.context.cluster.tags[placement_strategy.tag])
                * placement_strategy.channels_per_node
                * (
                    self.context.io_per_node
                    if node_type == "input"
    ...
    def new_input_dataset_node(self, dataset, stage=0):
        ...
        for ip in objects_dict:
            ...
            for i in range(len(nodes)):
                ...
                objects = list(
                    range(
                        i * objects_per_node,
                        min((i + 1) * objects_per_node, len(objects_dict[ip])),
                    )
                )
                if len(objects) == 0:
                    break
                ...
                vals = {
                    pickle.dumps((self.current_actor, count, seq)): pickle.dumps(
                        lineages[seq]
                    )
                    for seq in range(len(lineages))
                }
                input_task = TapedInputTask(
                    self.current_actor, count, [i for i in range(len(lineages))]
                )
                self.LIT.set(
                    pipe, pickle.dumps((self.current_actor, count)), len(lineages) - 1
                )
        ...
        pipe.execute()
        ray.get(
            self.context.coordinator.register_actor_location.remote(
    ...
    def new_input_reader_node(
        self, reader: Reader, stage: StageId = 0, placement_strategy=None
    ):
        ...
        if type(placement_strategy) == SingleChannelStrategy:
            ...
            type(placement_strategy) == CustomChannelsStrategy
            or type(placement_strategy) == TaggedCustomChannelsStrategy
        ):

            nodes = (
                sorted(self.context.io_nodes)
                if type(placement_strategy) == CustomChannelsStrategy
                else sorted(self.context.tag_io_nodes[placement_strategy.tag])
            )
            ...
            for node in nodes:
                for channel in range(placement_strategy.channels_per_node):
                    ...
                    if len(lineages) > 0:
                        vals = {
                            pickle.dumps(
                                (self.current_actor, count, seq)
                            ): pickle.dumps(lineages[seq])
                            for seq in range(len(lineages))
                        }
                        input_task = TapedInputTask(
                            self.current_actor, count, [i for i in range(len(lineages))]
                        ...
                        self.LIT.set(
                            pipe,
                            pickle.dumps((self.current_actor, count)),
                            len(lineages) - 1,
                        )
                        self.LT.mset(pipe, vals)
                        self.NTT.rpush(pipe, node, input_task.reduce())
                        ...
                        self.DST.set(
                            pipe, pickle.dumps((self.current_actor, count)), -1
                        )
    ...
    def get_default_partition(self, source_node_id, target_placement_strategy):
        ...
        if (
            type(source_placement_strategy) == CustomChannelsStrategy
            or type(source_placement_strategy) == TaggedCustomChannelsStrategy
        ):
        ...
        if source_total_channels >= target_total_channels:
            return partial(
                partition_key_0, source_total_channels // target_total_channels
            )
        else:
    ...
    def prologue(
        ...
        def partition_key_str(key, data, source_channel, num_target_channels):
            ...
            if "int" in str(data[key].dtype).lower():
                partitions = data.with_columns(
                    polars.Series(
                        name="__partition__", values=(data[key] % num_target_channels)
                    )
                ).partition_by("__partition__")
            elif (
                data[key].dtype == polars.datatypes.Utf8
                or data[key].dtype == polars.datatypes.Float32
                or data[key].dtype == polars.datatypes.Float64
            ):
                partitions = data.with_columns(
                    polars.Series(
                        name="__partition__",
                        values=(data[key].hash() % num_target_channels),
                    )
                ).partition_by("__partition__")
            else:
                print(data[key])
        ...
        def partition_key_range(
            key, total_range, data, source_channel, num_target_channels
        ):
            ...
            partitions = data.with_columns(
                polars.Series(
                    name="__partition__", values=((data[key] - 1) // per_channel_range)
                )
            ).partition_by("__partition__")
            for partition in partitions:
                target = partition["__partition__"][0]
        ...
        for key in streams:
            ...
            if type(target_info.partitioner) == HashPartitioner:
                ...
                target_info.partitioner = partial(
                    partition_key_range,
                    target_info.partitioner.key,
                    target_info.partitioner.total_range,
                )
            ...
            self.PFT.set(
                self.r,
                pickle.dumps((source, self.current_actor)),
                ray.cloudpickle.dumps(target_info),
            )
        ...
        registered = ray.get(
            [
                node.register_partition_function.remote(
                    sources,
                    self.current_actor,
                    self.get_total_channels_from_placement_strategy(
                        placement_strategy, "exec"
                    ),
                    mapping,
                )
                for node in (self.context.task_managers.values())
            ]
        )
        ...
        for stage in sorted(stage_sources.keys()):
            ...
            input_reqs.append(
                polars.from_dict(
                    {
                        "source_actor_id": source_actor_ids,
    ...
    def new_non_blocking_node(
        ...
        placement_strategy=CustomChannelsStrategy(1),
        source_target_info: dict[SourceDataStreamIndex, TargetInfo] = {},
        assume_sorted: dict[SourceDataStreamIndex, bool | int] = {},
    ):
        ...
        for key in assume_sorted:
            ...
            sorted_source_channels = len(
                polars.concat(input_reqs).filter(polars.col("source_actor_id") == key)
            )
        ...
        if type(placement_strategy) == SingleChannelStrategy:
            ...
            self.CLT.set(
                pipe,
                pickle.dumps((self.current_actor, 0)),
                self.context.node_locs[node],
            )
            self.IRT.set(
                pipe,
                pickle.dumps((self.current_actor, 0, -1)),
                pickle.dumps(input_reqs),
            )
            ...
            nodes = (
                sorted(self.context.compute_nodes)
                if type(placement_strategy) == CustomChannelsStrategy
            ...
            for node in nodes:
                for channel in range(placement_strategy.channels_per_node):
                    ...
                    self.IRT.set(
                        pipe,
                        pickle.dumps((self.current_actor, count, -1)),
                        pickle.dumps(input_reqs),
                    )
                    count += 1
    ...
    def create(self):
        ...
        initted = ray.get(
            [node.init.remote() for node in list(self.context.task_managers.values())]
        )
        assert all(initted)

...
from pyquokka.types import ChannelId, ColumnName, IPartitioner
...
class RangePartitioner(Partitioner):
    ...
    def __str__(self):
        return (
            "range partitioner on "
            + str(self.key)
            + ", range estimate "
            + str(self.total_range)
        )
...
class TargetInfo:
    def __init__(
        ...
        projection: Optional[Iterable[ColumnName]],
        batch_funcs: list,
    ) -> None:
        ...
        self.partitioner: (
            Partitioner
            | Callable[[polars.DataFrame, Any, int], dict[ChannelId, polars.DataFrame]]
        ) = partitioner
    ...
    def __str__(self) -> str:
        return (
            "partitioner: "
            + str(self.partitioner)
            + "\n\t  predicate: "
            + self.predicate.sql()
            + "\n\t  projection: "
            + str(self.projection)
            + "\n\t batch_funcs: "
            + str(self.batch_funcs)
        )

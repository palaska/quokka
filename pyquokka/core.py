...
class TaskManager:
    ...
    def register_partition_function(
        self, source_actor_ids, target_actor_id, number_target_channels, mapping
    ):
        ...
        for source_actor_id in source_actor_ids:
            target_info = ray.cloudpickle.loads(
...
class ExecTaskManager(TaskManager):
    ...
    def process_output(
        self, actor_id, channel_id, output, transaction, state_seq, out_seq
    ):
        ...
        while True:
            ...
            if self.configs["blocking"]:
                candidate_tasks = [
                    ...
                    if self.ast[pickle.loads(candidate_task)[1][0]]
                    <= self.current_stage
                ]

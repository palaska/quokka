...
from pyquokka.types import ChannelId
...
class InputEC2ParquetDataset:
    ...
    def __init__(
        ...
        if (
            self.name_column is not None
            and self.columns is not None
            and self.name_column in self.columns
        ):
            self.columns.remove(self.name_column)

    def get_own_state(self, num_channels: int) -> dict[ChannelId, list[list[str]]]:
        ...
        for channel in range(num_channels):
            my_files = [
                self.files[k]
                for k in range(channel, len(self.files), self.num_channels)
            ]
    ...
    def execute(self, mapper_id, files_to_do=None):
            ...
            with warnings.catch_warnings():
                ...
                if self.name_column is not None:
                    result = result.append_column(
                        self.name_column,
                        pa.array(
                            [file.split("/")[-1].replace(".parquet", "")] * len(result)
                        ),
                    )
                return result
        ...
        future_to_url = {
            self.executor.submit(download, file): file for file in files_to_do
        }
        dfs = []
...
class InputLanceDataset:
    ...
    def get_own_state(self, num_channels):
        ...
        for channel in range(num_channels):
            my_files = [
                self.uris[k] for k in range(channel, len(self.uris), self.num_channels)
            ]
            channel_infos[channel] = []
            for pos in range(0, len(my_files), self.workers):
                channel_infos[channel].append(my_files[pos : pos + self.workers])
                ...
                results = pa.concat_tables(
                    [
                        dataset.to_table(
                            ...
                            filter=self.filters,
                            nearest={
                                "column": self.vec_column,
                                "k": self.k,
                                "q": self.probe_df_vecs[i],
                            },
                        )
                        for i in range(len(self.probe_df_vecs))
                    ]
                )
            else:
    ...
    def get_next_batch(self, mapper_id, pos=None):
        ...
        while curr_pos < len(self.files):
            ...
            a = pa.Table.from_pydict(
                {
                    "filename": [self.files[curr_pos]],
                    "object": [
                        s3.get_object(Bucket=self.bucket, Key=self.files[curr_pos])[
                            "Body"
                        ].read()
                    ],
                }
            ...
            a = pa.Table.from_pydict(
                {
                    "filename": [self.files[curr_pos]],
                    "object": [
                        open(self.directory + "/" + self.files[curr_pos], "rb").read()
                    ],
                }
            )
            curr_pos += self.num_channels
...
class InputDiskCSVDataset:
    def __init__(
        ...
        self.window = window
        assert not (
            names is None and header is False
        ), "if header is False, must supply column names"
    ...
    def get_own_state(self, num_channels: int):
        ...
        if os.path.isfile(self.filepath):
            ...
            assert os.path.isdir(
                self.filepath
            ), "Does not support prefix, must give absolute directory path for a list of files, will read everything in there!"
            files = deque(
                [self.filepath + "/" + file for file in os.listdir(self.filepath)]
            )
        ...
        if self.header:
            ...
            if first_newline == -1:
                raise Exception(
                    "could not detect the first line break. try setting the window argument to a large number"
                )
            if self.names is None:
                ...
                if self.names != detected_names:
                    print(
                        "Warning, detected column names from header row not the same as supplied column names! This could also be because your rows end with the delimiter."
                    )
        ...
        if self.sort_key is not None:
            ...
            for file, size in zip(files, sizes):
                ...
                min_key = csv.read_csv(
                    BytesIO(resp),
                    read_options=csv.ReadOptions(column_names=self.names),
                    parse_options=csv.ParseOptions(delimiter=self.sep),
                )[self.sort_key][0].as_py()
                f = open(file, "rb")
                f.seek(size - self.window)
                resp = f.read(self.window)
                first_newline = resp.find(bytes("\n", "utf-8"))
                resp = resp[first_newline + 1 :]
                max_key = csv.read_csv(
                    BytesIO(resp),
                    read_options=csv.ReadOptions(column_names=self.names),
                    parse_options=csv.ParseOptions(delimiter=self.sep),
                )[self.sort_key][-1].as_py()
            ...
            for i in range(1, len(file_stats)):
                assert (
                    overlap(
                        [file_stats[i - 1][1], file_stats[i - 1][2]],
                        [file_stats[i][1], file_stats[i][2]],
                    )
                    <= 0
                ), "data is not sorted!"
        ...
        channel_info: dict[ChannelId, list[tuple[str, int, str, int]]] = {}
        if self.sort_key is None:
            ...
            for channel in range(num_channels):
                channel_info[channel] = [
                    partitions[channel * partitions_per_channel + i]
                    for i in range(partitions_per_channel)
            ...
            for channel in range(num_channels):
                channel_info[channel] = [
                    partitions[i]
                    for i in range(channel, len(partitions), num_channels)
                    if i in partitions
                ]
    ...
    def execute(self, mapper_id, state=None):
        ...
        bytes_to_read = min(
            min(start_byte + self.stride, end) - start_byte, size_per_partition
        )
        ...
        if last_newline == -1:
            ...
            bump = polars.read_csv(
                resp,
                new_columns=self.names,
                separator=self.sep,
                has_header=False,
...
class InputDiskJSONDataset:
    def __init__(
        ...
        if schema is not None:
            self.parse_options = pajson.ParseOptions(
                pajson.ParseOptions(explicit_schema=schema, newlines_in_values=False)
            )
        else:
            self.parse_options = pajson.ParseOptions(
                pajson.ParseOptions(newlines_in_values=False)
            )
...
class FakeFile:
    ...
    def read(self, length):
        if self.file_cursor + length < self.end:
            if self.is_first_read:
                ...
                return self.buffers[self.which_file][
                    self.file_cursor - length : self.file_cursor
                ]
        else:
    ...
    def get_end(self):
        return self.buffers[self.end_file][
            self.last_newline : len(self.buffers[self.end_file])
        ]
...
class InputS3CSVDataset:
    ...
    def get_own_state(self, num_channels):
        ...
        s3 = boto3.client(
            "s3"
        )  # needs boto3 client, however it is transient and is not part of own state, so Ray can send this thing!
        ...
        if self.header:
            resp = s3.get_object(
                Bucket=self.bucket,
                Key=files[0],
                Range="bytes={}-{}".format(0, self.window),
            )["Body"].read()
            first_newline = resp.find(bytes("\n", "utf-8"))
            if first_newline == -1:
                raise Exception(
        ...
        total_size = sum(sizes)
        assert total_size > 0
        size_per_partition = min(
            int(self.stride * self.workers), math.ceil(total_size / num_channels)
        )
        # size_per_partition = int(self.stride * workers)
        ...
        def download_ranges(inputs):
            ...
            for partition in inputs:
                ...
                futures[partition] = executor.submit(
                    download_range, bucket, file, start_byte, end_byte
                )
        ...
        for i in range(len(ips)):
            ip = ips[i]
            prefixes_futs.append(
                download_ranges.options(resources={ip: 0.001}).remote(
                    {
                        partition: inputs[partition]
                        for partition in partition_list[
                            i * prefixes_per_ip : (i + 1) * prefixes_per_ip
                        ]
                    }
                )
            )
        ...
        self.file_sizes = {files[i]: sizes[i] for i in range(len(files))}
        print(
            "initialized CSV reading strategy for ",
    ...
    def execute(self, mapper_id, state=None):
        ...
        def download(x):
            ...
            end_byte = min(
                pos + x * self.stride + self.stride - 1,
                end - 1,
        ...
        future_to_url = {
            self.executor.submit(download, x): x for x in range(self.workers)
        }
        results = {}
        for future in concurrent.futures.as_completed(future_to_url):

...
class FlightServer(pyarrow.flight.FlightServerBase):
    ...
    def _all_done(self, target_id, target_channel):
        if not all(
            k == "done"
            for k in self.latest_input_received[target_id, target_channel].values()
        ):
    ...
    def do_get(self, context, ticket):
        ...
        mode, actor_id, channel_id, input_requirements, exact, sorted_reqs = (
        ...
        if not exact:
            ...
            if self.flight_keys is None or len(self.flight_keys) == 0:
                ...
                self.flights_lock.release()
                return pyarrow.flight.GeneratorStream(
                    pyarrow.schema([]), self.number_batches([])
                )

            exec_plan = (
                self.flight_keys.lazy()
                .filter(
                    (polars.col("target_actor_id") == actor_id)
                    & (polars.col("target_channel_id") == channel_id)
                )
                .join(
                    input_requirements.lazy(),
                    left_on=["source_actor_id", "source_channel_id"],
                    right_on=["source_actor_id", "source_channel_id"],
                )
                .join(
                    self.config_dict["actor_stages"].lazy(),
                    left_on="source_actor_id",
                    right_on="actor_id",
                )
            ...
            exec_plan = (
                exec_plan.filter(polars.col("stage") == min_stage)
                .sort("seq")
                .limit(self.config_dict["max_batches"])
                .select(
                    [
                        "source_actor_id",
                        "source_channel_id",
            ...
            if len(exec_plan) < self.config_dict["batch_attempt"]:
                if ticket.ticket not in self.requests_hashes:
                    ...
                    ticket.ticket in self.requests_hashes
                    and len(exec_plan) > self.requests_hashes[ticket.ticket]
                ):
                    self.requests_hashes[ticket.ticket] = len(exec_plan)
                    self.flights_lock.release()
                    return pyarrow.flight.GeneratorStream(
            ...
            source_actor_id = (
                exec_plan.groupby("source_actor_id")
                .agg(polars.count())
                .sort("count", descending=True)
                .limit(1)
                .to_dicts()[0]["source_actor_id"]
            )
            ...
            exec_plan_candidate = exec_plan.filter(
                polars.col("source_actor_id") == source_actor_id
            )
            ...
            if sorted_reqs is None or source_actor_id not in sorted_reqs:
                ...
                for source_channel_id, df in exec_plan_candidate.groupby(
                    "source_channel_id"
                ):
                ...
                input_requirements = input_requirements.filter(
                    polars.col("source_actor_id") == source_actor_id
                )
                stuff = (
                    input_requirements.sort("source_channel_id")
                    .with_columns(
                        (
                            input_requirements["min_seq"]
                            - input_requirements["min_seq"].shift(1)
                        ).alias("lag")
                    )
                ...
                if len(stuff) == 0:
                    ...
                    start_channel_id = input_requirements.sort(
                        "source_channel_id"
                    ).to_dicts()[0]["source_channel_id"]
                    # assert start_channel_id == 0, "start channel id must be zero"
                    curr_seq_no = input_requirements.sort(
                        "source_channel_id"
                    ).to_dicts()[0]["min_seq"]
                else:
        ...
        return pyarrow.flight.GeneratorStream(
            schema, self.number_batches(batches, fake_row=fake_row)
        )
    ...
    def do_action(self, context, action):
        if action.type == "clear":
            ...
            for key in config_dict:
                ...
                ), "got an unrecognized Flight server config"
                if key == "actor_stages":
                    self.config_dict[key] = polars.DataFrame(
                        list(config_dict[key].items()),
                        orient="row",
                        schema=["actor_id", "stage"],
                    )
                else:
            ...
            ) > self.config_dict["mem_limit"]
            yield pyarrow.flight.Result(pyarrow.py_buffer(bytes(str(cond), "utf-8")))

        elif action.type == "get_flights_info":

            yield pyarrow.flight.Result(
                pyarrow.py_buffer(
                    pickle.dumps((self.flight_keys, list(self.flights.keys())))
                )
            )
            ...
            self.flight_keys = self.flight_keys.join(
                gcable,
                on=[
                    "source_actor_id",
                    "source_channel_id",

...
def target_info_to_transform_func(target_info):
    ...
    if target_info.predicate == sqlglot.exp.TRUE:
        return partial(
            transform_fn_no_filter, target_info.batch_funcs, target_info.projection
        )
    elif target_info.predicate == sqlglot.exp.FALSE:
        print(
            "false predicate detected, entire subtree is useless. We should cut the entire subtree!"
        )
        return lambda x: None
    else:
        predicate = sql_utils.evaluate(target_info.predicate)
        return partial(
            transform_fn, predicate, target_info.batch_funcs, target_info.projection
        )
...
class Node(INode):
    def __init__(self, schema: Schema) -> None:
        ...
        self.targets: Dict[NodeId, TargetInfo] = {}
        self.parents: Dict[SourceDataStreamIndex, NodeId] = {}

        self.blocking = False
        self.placement_strategy: Optional[PlacementStrategy] = None
        ...
        self.cardinality = {}
        self.stage: Optional[StageId] = None

    def assign_stage(self, stage: StageId):
        self.stage = stage
    ...
    def lower(self, task_graph: TaskGraph) -> TaskGraphNodeId:
        raise NotImplementedError

    def set_output_sorted_reqs(self, reqs):
        self.output_sorted_reqs = reqs

    def set_placement_strategy(self, strategy: PlacementStrategy):
...
class SourceNode(Node):
    def __init__(self, schema: Schema) -> None:
        ...
        self.catalog_id: Optional[CatalogTableId] = None
    ...
    def set_catalog_id(self, catalog_id: CatalogTableId):
...
class InputS3FilesNode(SourceNode):
    def __init__(self, bucket, prefix, schema: Schema) -> None:
        super().__init__(schema)
        self.bucket = bucket
        self.prefix = prefix
    ...
    def lower(self, task_graph):
        ...
        node = task_graph.new_input_reader_node(
            file_reader, self.stage, self.placement_strategy
        )
        return node


class InputDiskFilesNode(SourceNode):
    def __init__(self, directory, schema: Schema) -> None:
        super().__init__(schema)
...
class InputPolarsNode(SourceNode):
    ...
    def lower(self, task_graph):
        assert self.stage is not None
        node = task_graph.new_input_reader_node(
            InputPolarsDataset(self.df), self.stage, SingleChannelStrategy()
        )
        return node
...
class InputS3CSVNode(SourceNode):
    def __init__(
        self, bucket, prefix, key, schema: Schema, sep, has_header, projection=None
    ) -> None:
        super().__init__(schema)
        self.bucket = bucket
        self.prefix = prefix
    ...
    def lower(self, task_graph):
        ...
        node = task_graph.new_input_reader_node(
            csv_reader, self.stage, self.placement_strategy
        )
        return node
...
class InputRestGetAPINode(SourceNode):
    def __init__(
        self, url, arguments, headers, schema: Schema, batch_size=10000
    ) -> None:
        super().__init__(schema)
        self.url = url
    ...
    def lower(self, task_graph):
        assert self.stage is not None
        rest_reader = InputRestGetAPIDataset(
            self.url, self.arguments, self.headers, self.schema, self.batch_size
        )
        node = task_graph.new_input_reader_node(
            rest_reader, self.stage, self.placement_strategy
        )
        return node
...
class InputRestPostAPINode(SourceNode):
    ...
    def lower(self, task_graph):
        assert self.stage is not None
        rest_reader = InputRestPostAPIDataset(
            self.url, self.arguments, self.headers, self.schema, self.batch_size
        )
        node = task_graph.new_input_reader_node(
            rest_reader, self.stage, self.placement_strategy
...
class InputDiskCSVNode(SourceNode):
    def __init__(
        self, filename, schema: Schema, sep, has_header, projection=None
    ) -> None:
        super().__init__(schema)
    ...
    def set_cardinality(self, catalog: ICatalog):

        assert self.catalog_id is not None
        for target in self.targets:
            predicate = sql_utils.label_sample_table_names(
                self.targets[target].predicate
            )
            self.cardinality[target] = ray.get(
                catalog.estimate_cardinality.remote(self.catalog_id, predicate)
            )
    ...
    def lower(self, task_graph):
        ...
        if self.output_sorted_reqs is not None:
            ...
            csv_reader = InputDiskCSVDataset(
                self.filename,
                self.schema,
                sep=self.sep,
                header=self.has_header,
...
class InputS3IcebergNode(SourceNode):
    def __init__(self, table, snapshot=None, predicate=None, projection=None) -> None:
        ...
        schema: Schema = [i.name for i in table.schema().fields]
        super().__init__(schema)
        self.table = table  # pyiceberg table format
        self.files = [
            task.file.file_path
            for task in self.table.scan(snapshot_id=snapshot).plan_files()
        ]
    ...
    def set_cardinality(self, catalog):
        ...
        if self.snapshot is None:
            cardinality = self.table.metadata.snapshots[
                -1
            ].summary.additional_properties["total-records"]
        else:
    ...
    def lower(self, task_graph):
        ...
        parquet_reader = InputEC2ParquetDataset(
            bucket=None,
            prefix=None,
            files=self.files,
            columns=list(self.projection) if self.projection is not None else None,
            filters=self.predicate,
        )
        node = task_graph.new_input_reader_node(
            parquet_reader, self.stage, self.placement_strategy
        )
        return node
...
class InputLanceNode(SourceNode):
    def __init__(
        self, uris, schema: Schema, vec_column, predicate=None, projection=None
    ) -> None:
        super().__init__(schema)
        self.uris = uris
        self.predicate = predicate
    ...
    def set_probe_df(self, probe_df, probe_df_col, k):
        ...
        assert (
            probe_df is not None and probe_df_col is not None and k is not None
        ), "must provide probe_df, probe_df_col, and k"

        assert type(probe_df) == polars.DataFrame
        self.probe_df = probe_df
        assert (
            type(probe_df_col) == str and probe_df_col in probe_df.columns
        ), "probe_df_col must be a string and in probe_df"
    ...
    def lower(self, task_graph):
        assert self.stage is not None
        lance_reader = InputLanceDataset(
            self.uris,
            self.vec_column,
            columns=list(self.projection) if self.projection is not None else None,
            filters=self.predicate,
        ...
        if self.probe_df is not None:
            operator = DFProbeDataStreamNNExecutor2(
                self.vec_column, self.probe_df, self.probe_df_col, self.k
            )
            target_info = TargetInfo(BroadcastPartitioner(), sqlglot.exp.TRUE, None, [])
            node = task_graph.new_non_blocking_node(
                {0: node},
                operator,
                self.stage,
...
class InputS3ParquetNode(SourceNode):
    ...
    def set_cardinality(self, catalog):
        ...
        for target in self.targets:
            ...
            self.cardinality[target] = ray.get(
                catalog.estimate_cardinality.remote(
                    self.catalog_id, predicate, self.predicate
                )
            )
    ...
    def lower(self, task_graph):
        ...
        if self.output_sorted_reqs is not None:
            ...
            key = list(self.output_sorted_reqs.keys())[0]
            val = self.output_sorted_reqs[key]
            parquet_reader = InputSortedEC2ParquetDataset(
                self.files,
                key,
                columns=list(self.projection) if self.projection is not None else None,
                filters=self.predicate,
...
class InputDiskParquetNode(SourceNode):
    def __init__(
        self, filepath, schema: Schema, predicate=None, projection=None
    ) -> None:
        super().__init__(schema)
    ...
    def lower(self, task_graph):
        ...
        if type(task_graph.context.cluster) == EC2Cluster:
            ...
            parquet_reader = InputParquetDataset(
                self.filepath, columns=list(self.projection), filters=self.predicate
            )
            node = task_graph.new_input_reader_node(
                parquet_reader, self.stage, self.placement_strategy
            )
...
class DataSetNode(SinkNode):
    ...
    def lower(self, task_graph, parent_nodes, parent_source_info):
        ...
        return task_graph.new_blocking_node(
            parent_nodes,
            StorageExecutor(),
            self.stage,
            self.placement_strategy,
            source_target_info=parent_source_info,
        )
...
class TaskNode(Node):
    def __init__(
        self, schema: Schema, schema_mapping: dict, required_columns: set
    ) -> None:
        super().__init__(schema)
...
class JoinNode(TaskNode):
    ...
    def add_join_spec(self, join_spec: JoinSpec):
        self.join_specs.append(join_spec)

    def __str__(self):
        result = (
            "Join Node with joins: "
            + str(self.join_specs)
            + "\n"
            + str(self.stage)
            + "\nParents:"
    ...
    def lower(self, task_graph, parent_nodes, parent_source_info):
        ...
        print(f"join specs {self.join_specs}")
        ...
        operator = BuildProbeJoinExecutor(
            None, left_key, right_key, join_type, key_to_keep
        )
        # operator = ExpBuildProbeJoinExecutor(None, left_key, right_key, join_type, key_to_keep)
        ...
        if len(self.join_specs) == 1 and self.blocking:
            return task_graph.new_blocking_node(
                {0: parent_nodes[left_parent], 1: parent_nodes[right_parent]},
                operator,
                self.stage,
                self.placement_strategy,
                source_target_info={
                    0: left_parent_target_info,
                    1: right_parent_target_info,
                },
                transform_fn=transform_func,
            )
        else:
            intermediate_node = task_graph.new_non_blocking_node(
                {0: parent_nodes[left_parent], 1: parent_nodes[right_parent]},
                operator,
                self.stage,
                self.placement_strategy,
        ...
        for i in range(1, len(self.join_specs)):
            ...
            intermediate_target_info = TargetInfo(
                HashPartitioner(left_key), sqlglot.exp.TRUE, None, []
            )
            ...
            if i == len(self.join_specs) - 1 and self.blocking:
                return task_graph.new_blocking_node(
                    {0: intermediate_node, 1: parent_nodes[right_parent]},
                    operator,
                    self.stage,
                    self.placement_strategy,
                    source_target_info={
                        0: intermediate_target_info,
                        1: parent_target_info,
                    },
                    transform_fn=transform_func,
                )
            else:
...
class BroadcastJoinNode(TaskNode):
    ...
    def lower(self, task_graph, parent_nodes, parent_source_info):
        ...
        if self.blocking:
            ...
            return task_graph.new_non_blocking_node(
                parent_nodes,
                self.operator,
                self.stage,
                self.placement_strategy,
...
class MapNode(TaskNode):
    def __init__(
        self, schema: Schema, schema_mapping, required_columns, function, foldable=True
    ) -> None:
        super().__init__(schema, schema_mapping, required_columns)
...
class FilterNode(TaskNode):
    ...
    def __init__(self, schema: Schema, predicate: sqlglot.Expression) -> None:
        super().__init__(
            schema=schema,
            schema_mapping={column: {0: column} for column in schema},
            required_columns={
                0: set(i.name for i in predicate.find_all(sqlglot.expressions.Column))
            },
        )
        self.predicate = predicate
    ...
    def lower(self, task_graph, parent_nodes, parent_source_info):
        print(
            "Tried to lower a filter node. This means the optimization probably failed and something bad is happening."
        )
        raise NotImplementedError
...
class NearestNeighborFilterNode(TaskNode):
    ...
    def __str__(self):
        result = "nearest neighbor filter node on vec column {} with {} probe vectors".format(
            self.vec_column, len(self.probe_df)
        )
        return result
    ...
    def set_cardinality(self, parent_cardinalities):
        ...
        for target in self.targets:
            if (
                self.targets[target].predicate == sqlglot.exp.TRUE
                and parent_cardinality is not None
            ):
    ...
    def lower(self, task_graph, parent_nodes, parent_source_info):
        ...
        executor1 = DFProbeDataStreamNNExecutor1(
            self.vec_column, self.probe_df, self.probe_vector_col, self.k
        )
        executor2 = DFProbeDataStreamNNExecutor2(
            self.vec_column, self.probe_df, self.probe_vector_col, self.k
        )

        intermediate_target_info = TargetInfo(
            BroadcastPartitioner(), sqlglot.exp.TRUE, None, []
        )
        ...
        if self.blocking:
            ...
            target_info = self.targets[list(self.targets.keys())[0]]
            transform_func = target_info_to_transform_func(target_info)
            intermediates = task_graph.new_non_blocking_node(
                parent_nodes,
                executor1,
                self.stage,
                self.placement_strategy,
            ...
            return task_graph.new_blocking_node(
                ...
                self.stage,
                SingleChannelStrategy(),
                source_target_info={0: intermediate_target_info},
                transform_fn=transform_func,
            )

...
class QuokkaContext(IQuokkaContext):
    def __init__(self, cluster=None, io_per_node=2, exec_per_node=1) -> None:
        ...
        self.sql_config = {
            "optimize_joins": True,
            "s3_csv_materialize_threshold": 10 * 1048576,
            "disk_csv_materialize_threshold": 1048576,
            "s3_parquet_materialize_threshold": 10 * 1048576,
            "disk_parquet_materialize_threshold": 1048576,
        }
        self.exec_config = {
            "hbq_path": "/data/",
            ...
            "checkpoint_bucket": "quokka-checkpoint",
            "batch_attempt": 20,
            "max_pipeline": 300,
        }
        ...
        self.coordinator = Coordinator.options(
            num_cpus=0.001,
            max_concurrency=2,
            resources={"node:" + str(self.cluster.leader_private_ip): 0.001},
        ).remote()
        self.catalog = Catalog.options(
            num_cpus=0.001,
            resources={"node:" + str(self.cluster.leader_private_ip): 0.001},
        ).remote()
        self.dataset_manager = ArrowDataset.options(
            num_cpus=0.001,
            resources={"node:" + str(self.cluster.leader_private_ip): 0.001},
        ).remote()
        ...
        for ip in private_ips:
            ...
            for k in range(1):
                self.task_managers[count] = ReplayTaskManager.options(
                    num_cpus=0.001, max_concurrency=2, resources={"node:" + ip: 0.001}
                ).remote(
                    count,
                    self.cluster.leader_public_ip,
                    list(self.cluster.private_ips.values()),
                    self.exec_config,
                )
            ...
            for k in range(io_per_node):
                self.task_managers[count] = IOTaskManager.options(
                    num_cpus=0.001, max_concurrency=2, resources={"node:" + ip: 0.001}
                ).remote(
                    count,
                    self.cluster.leader_public_ip,
            ...
            for k in range(exec_per_node):
                if type(self.cluster) == LocalCluster:
                    self.task_managers[count] = ExecTaskManager.options(
                        num_cpus=0.001,
                        max_concurrency=2,
                        resources={"node:" + ip: 0.001},
                    ).remote(
                        count,
                        self.cluster.leader_public_ip,
                        list(self.cluster.private_ips.values()),
        ...
        ray.get(
            self.coordinator.register_nodes.remote(
                replay_nodes={k: self.task_managers[k] for k in self.replay_nodes},
                io_nodes={k: self.task_managers[k] for k in self.io_nodes},
                compute_nodes={k: self.task_managers[k] for k in self.compute_nodes},
            )
        )
    ...
    def set_config(self, key, value):
        ...
        if key in self.sql_config:
            ...
            assert all(
                ray.get(
                    [
                        task_manager.set_config.remote(key, value)
    ...
    def read_files(self, table_location: str):
        ...
        if table_location[:5] == "s3://":
            ...
            if type(self.cluster) == LocalCluster:
                print(
                    "Warning: trying to read S3 dataset on local machine. This assumes high network bandwidth."
                )
            ...
            if "*" in table_location:
                assert (
                    table_location[-1] == "*"
                ), "wildcard can only be the last character in address string"
                prefix = "/".join(table_location[:-1].split("/")[1:])
                self.nodes[self.latest_node_id] = InputS3FilesNode(
                    bucket, prefix, ["filename", "object"]
                )
            else:
                raise NotImplemented(
                    "Are you trying to read a single file? It's not supported. Please use s3://bucket/prefix*. Add the asterisk!"
                )
        else:
            if type(self.cluster) == EC2Cluster:
                raise NotImplementedError(
                    "Does not support reading local dataset with S3 cluster. Must use S3 bucket."
                )
            ...
            if "*" in table_location:
                ...
                assert (
                    table_location[-1] == "/"
                ), "must specify * with entire directory, doesn't support prefixes yet"
                assert os.path.isdir(table_location), "must supply absolute path"

                self.nodes[self.latest_node_id] = InputDiskFilesNode(
                    table_location, ["filename", "object"]
                )
                ...
                raise NotImplemented(
                    "Are you trying to read a single file? It's not supported. Please supply absolute directory to the files, like /tmp/*. Add the asterisk!"
                )
    ...
    def read_csv(
        ...
        def get_schema_from_bytes(resp: bytes) -> Schema:
        ...
        def return_materialized_stream(
            where: str, my_schema: Optional[Schema] = None
        ) -> DataStream:
            if has_header:
                df = polars.read_csv(where, has_header=True, separator=sep)
            else:
                df = polars.read_csv(
                    where, new_columns=my_schema, has_header=False, separator=sep
                )
            self.nodes[self.latest_node_id] = InputPolarsNode(df)
            self.latest_node_id += 1
            return DataStream(
                self, df.columns, self.latest_node_id - 1, materialized=True
            )
        ...
        if schema is not None and has_header:
            print(
                "You specified a schema as well as a header. Quokka should use your schema and ignore the names in the header."
            )
        ...
        if table_location[:5] == "s3://":
            ...
            if "*" in table_location:
                ...
                files = [i["Key"] for i in z["Contents"]]  # type: ignore
                sizes = [i["Size"] for i in z["Contents"]]  # type: ignore
                assert "NextContinuationToken" not in z, "too many files in S3 bucket"
                assert len(files) > 0, "no files under prefix"

                if schema is None:
                    resp = s3.get_object(
                        Bucket=bucket, Key=files[0], Range="bytes={}-{}".format(0, 4096)
                    )["Body"].read()
                ...
                if (
                    len(files) == 1
                    and sizes[0] < self.sql_config["s3_csv_materialize_threshold"]
                ):
                    return return_materialized_stream(
                        "s3://" + bucket + "/" + files[0], schema
                    )

                token = cast(
                    CatalogTableId,
                    ray.get(
                        self.catalog.register_s3_csv_source.remote(
                            bucket, files[0], schema, sep, sum(sizes)
                        )
                    ),
                )
                self.nodes[self.latest_node_id] = InputS3CSVNode(
                    bucket, prefix, None, schema, sep, has_header
                )
                cast(SourceNode, self.nodes[self.latest_node_id]).set_catalog_id(token)
                ...
                if schema is None:
                    resp = s3.get_object(
                        Bucket=bucket, Key=key, Range="bytes={}-{}".format(0, 4096)
                    )["Body"].read()
                    schema = get_schema_from_bytes(resp)
                ...
                token = ray.get(
                    self.catalog.register_s3_csv_source.remote(
                        bucket, key, schema, sep, size
                    )
                )
                self.nodes[self.latest_node_id] = InputS3CSVNode(
                    bucket, None, key, schema, sep, has_header
                )
            ...
            if "*" in table_location:
                ...
                try:
                    ...
                    raise Exception(
                        "Tried to get list of files at ",
                        table_location,
                        " failed. Make sure specify absolute path",
                    )
                assert len(files) > 0
                ...
                if (
                    len(files) == 1
                    and os.path.getsize(table_location + files[0])
                    < self.sql_config["disk_csv_materialize_threshold"]
                ):
                    return return_materialized_stream(table_location + files[0], schema)

                token = ray.get(
                    self.catalog.register_disk_csv_source.remote(
                        table_location, schema, sep
                    )
                )
                self.nodes[self.latest_node_id] = InputDiskCSVNode(
                    table_location, schema, sep, has_header
                )
                self.nodes[self.latest_node_id].set_catalog_id(token)
            else:
    ...
    def read_parquet(
        self, table_location: str, nthreads=4, name_column: Optional[ColumnName] = None
    ):
        """
        ...
        if table_location[:5] == "s3://":
            ...
            if "*" in table_location:
                ...
                sizes = [
                    i["Size"] for i in z["Contents"] if i["Key"].endswith(".parquet")
                ]
                while "NextContinuationToken" in z.keys():
                    z = s3.list_objects_v2(
                        Bucket=bucket,
                        Prefix=prefix,
                        ContinuationToken=z["NextContinuationToken"],
                    )
                    files.extend(
                        [
                            bucket + "/" + i["Key"]
                ...
                assert (
                    len(files) > 0
                ), "could not find any parquet files. make sure they end with .parquet"
                if (
                    sum(sizes) < self.sql_config["s3_parquet_materialize_threshold"]
                    and len(files) == 1
                ):
                    df = polars.from_arrow(
                        pq.read_table(files[0], filesystem=S3FileSystem())
                    )
                    if name_column is not None:
                        assert (
                            name_column not in df.columns
                        ), "name_column already exists in Parquet columns"
                        return return_materialized_stream(
                            df.with_columns(
                                polars.lit(
                                    files[0].split("/")[-1].replace(".parquet", "")
                                )
                            ).alias(name_column)
                        )
                    else:
                ...
                try:
                    ...
                    raise Exception(
                        "schema discovery failed for Parquet dataset at location {}. Please raise Github issue.".format(
                            table_location
                        )
                    )
                ...
                if name_column is not None:
                    assert (
                        name_column not in schema
                    ), "name_column already exists in Parquet columns"
                    schema.append(name_column)
                token = ray.get(
                    self.catalog.register_s3_parquet_source.remote(files[0], len(sizes))
                )
                self.nodes[self.latest_node_id] = InputS3ParquetNode(
                    files, schema, nthreads=nthreads, name_column=name_column
                )
                self.nodes[self.latest_node_id].set_catalog_id(token)
            else:
                try:
                    ...
                    raise Exception(
                        """schema discovery failed for Parquet dataset at location {}.
                                    Note if you are specifying a prefix to many parquet files, must use asterix. E.g.
                                    qc.read_parquet("s3://rottnest/happy.parquet/*")""".format(
                            table_location
                        )
                    )
                key = "/".join(table_location.split("/")[1:])
                ...
                if size < self.sql_config["s3_parquet_materialize_threshold"]:
                    df = polars.from_arrow(
                        pq.read_table(table_location, filesystem=S3FileSystem())
                    )
                    if name_column is not None:
                        assert (
                            name_column not in df.columns
                        ...
                        return return_materialized_stream(
                            df.with_columns(
                                polars.lit(key.split("/")[-1].replace(".parquet", ""))
                            ).alias(name_column)
                        )
                    else:
                        return return_materialized_stream(df)
                ...
                token = ray.get(
                    self.catalog.register_s3_parquet_source.remote(
                        bucket + "/" + key, 1
                    )
                )
                if name_column is not None:
                    ...
                    schema.append(name_column)
                self.nodes[self.latest_node_id] = InputS3ParquetNode(
                    [table_location], schema, nthreads=nthreads, name_column=name_column
                )
                self.nodes[self.latest_node_id].set_catalog_id(token)
            ...
            if name_column is not None:
                raise NotImplementedError(
                    "Does not support name_column for local dataset yet."
                )
            ...
            if "*" in table_location:
                ...
                try:
                    files = [
                        i for i in os.listdir(table_location) if i.endswith(".parquet")
                    ]
                except:
                    raise Exception(
                        "Tried to get list of parquet files at ",
                        table_location,
                        " failed. Make sure specify absolute path and filenames end with .parquet",
                    )
                assert len(files) > 0
                ...
                if (
                    len(files) == 1
                    and os.path.getsize(table_location + files[0])
                    < self.sql_config["disk_parquet_materialize_threshold"]
                ):
                    df = polars.read_parquet(table_location + files[0])
                    return return_materialized_stream(df)

                token = ray.get(
                    self.catalog.register_disk_parquet_source.remote(table_location)
                )
                self.nodes[self.latest_node_id] = InputDiskParquetNode(
                    table_location, schema
                )
                self.nodes[self.latest_node_id].set_catalog_id(token)
                ...
                try:
                    ...
                    raise Exception(
                        "could not find the parquet file at ", table_location
                    )
    ...
    def read_lance(self, table_location: str, vec_column: str):
        ...
        try:
            ...
            raise Exception(
                "lance not installed. pylance must be installed on each machine in cluster."
            )
        ...
        if table_location[:5] == "s3://":
            ...
            if "*" in table_location:
                ...
                files = [
                    "s3://" + bucket + "/" + i["Prefix"]
                    for i in z["CommonPrefixes"]
                    if i["Prefix"].endswith(".lance/")
                ]
                while "NextContinuationToken" in z.keys():
                    z = s3.list_objects_v2(
                        Bucket=bucket,
                ...
                assert (
                    len(files) > 0
                ), "could not find any lance files. make sure they end with .lance"
                ...
                try:
                    ...
                    raise Exception(
                        """schema discovery failed for Parquet dataset at location {}. Please raise Github issue.
                                    Lance dataset with S3 does not work with creds set with AWS configure, must use env variables.""".format(
                            table_location
                        )
                    )

                # token = ray.get(self.catalog.register_s3_lance_source.remote(files[0], len(sizes)))
                self.nodes[self.latest_node_id] = InputLanceNode(
                    files, schema, vec_column
                )
                # self.nodes[self.latest_node_id].set_catalog_id(token)
            else:
                try:
                ...
                self.nodes[self.latest_node_id] = InputLanceNode(
                    [table_location], schema, vec_column
                )
                # self.nodes[self.latest_node_id].set_catalog_id(token)
            ...
            if "*" in table_location:
                ...
                try:
                    files = [
                        i for i in os.listdir(table_location) if i.endswith(".lance")
                    ]
                except:
                    raise Exception(
                        "Tried to get list of parquet files at ",
    ...
    def read_rest_post(self, url, data, schema, headers={}, batch_size=10000):
        self.nodes[self.latest_node_id] = InputRestPostAPINode(
            url, data, headers, schema, batch_size=batch_size
        )
        self.latest_node_id += 1
        return DataStream(self, schema, self.latest_node_id - 1)

    def read_rest_get(self, url, data, schema, headers={}, batch_size=10000):
        self.nodes[self.latest_node_id] = InputRestGetAPINode(
            url, data, headers, schema, batch_size=batch_size
        )
        self.latest_node_id += 1
        return DataStream(self, schema, self.latest_node_id - 1)
    ...
    def read_ray_dataset(self, dataset, get_size=True):
        ...
        for arrow_ref in arrow_refs:
            my_node = ray.experimental.get_object_locations([arrow_ref])[arrow_ref][
                "node_ids"
            ]
            assert (
                len(my_node) == 1
            ), "arrow ref should only be on one node, please raise Github issue."
            my_node = my_node[0]
            node_ip = [i["NodeManagerAddress"] for i in nodes if i["NodeID"] == my_node]
            assert (
                len(node_ip) == 1
            ), "node id should only be on one IP, please raise Github issue."
            node_ip = node_ip[0]
            handles.append(
                self.dataset_manager.added_object.remote(
                    dataset_id, node_ip, [arrow_ref, size]
                )
            )
        ray.get(handles)
    ...
    def read_sorted_parquet(
        self,
        table_location: str,
        sorted_by: ColumnName,
        nthreads=4,
        sort_order: SortOrder = "stride",
        name_col: Optional[ColumnName] = None,
    ):
        assert type(sorted_by) == str
        assert sort_order in ["stride", "range"]
        stream = self.read_parquet(
            table_location, nthreads=nthreads, name_column=name_col
        )
        assert (
            sorted_by in stream.schema
        ), f"sorted_by column {sorted_by} not in schema {stream.schema}"
    ...
    def read_iceberg(self, table, snapshot=None):
        ...
        if snapshot is not None:
            assert snapshot in [
                i.snapshot_id for i in table.metadata.snapshots
            ], "snapshot not found"

        self.nodes[self.latest_node_id] = InputS3IcebergNode(table, snapshot)
        self.latest_node_id += 1
        return DataStream(
            self, [i.name for i in table.schema().fields], self.latest_node_id - 1
        )
    ...
    def mingle(self, sources: dict, operator, new_schema: list, required_cols: dict):
        ...
        assert (
            self.io_per_node == self.exec_per_node
        ), "mingle currently only supports 1 to 1 mapping of IO to exec nodes, since we don't identify data sources"
    ...
    def new_stream(
        self,
        sources: Dict[SourceDataStreamIndex, DataStream],
        partitioners: Dict[SourceDataStreamIndex, Partitioner],
        node: Node,
        schema: Schema,
        sorted: Optional[dict] = None,
        ...
        if sorted is None:
            return DataStream(
                self, schema, self.latest_node_id - 1, sorted, materialized
            )
        else:
            return OrderedStream(
                DataStream(self, schema, self.latest_node_id - 1, sorted, materialized),
                sorted,
            )
    ...
    def lower(
        self, end_node_id: NodeId, collect=True, dataset_schema=None
    ) -> Dataset | polars.DataFrame | polars.Series:
        ...
        while len(nodes) > 0:
            ...
            for parent_idx in new_node.parents:
                ...
                reverse_sorted_nodes.append(
                    (parent_id, self.execution_nodes[parent_id])
                )
        ...
        for node_id, node in reverse_sorted_nodes:
            if issubclass(type(node), SourceNode):
                ...
                target_info = {
                    parent_idx: self.execution_nodes[node.parents[parent_idx]].targets[
                        node_id
                    ]
                    for parent_idx in node.parents
                }
                task_graph_nodes[node_id] = node.lower(
                    task_graph, parent_nodes, target_info
                )
    ...
    def execute_node(
        ...
        node_id: NodeId,
        explain=False,
        mode: Optional[ExplainMode] = None,
        collect=True,
    ) -> Dataset | polars.DataFrame | polars.Series | None:
        ...
        for execute_node_id in self.execution_nodes:
            ...
            if len(new_targets) > 1:
                raise Exception(
                    "Currently a DataStream can only have one downstream consumer, please file an issue!"
                )
    ...
    def explain(self, node_id: NodeId, mode: Optional[ExplainMode] = "graph"):
        ...
        if mode == "text":
            ...
            logical_plan_graph.node(
                str(node_id), str(node_id) + str(self.execution_nodes[node_id])
            )
            for parent in self.execution_nodes[node_id].parents:
                self._walk(
                    self.execution_nodes[node_id].parents[parent], logical_plan_graph
                )
                try:
                    cardinality = self.execution_nodes[
                        self.execution_nodes[node_id].parents[parent]
                    ].cardinality[node_id]
                    cardinality = (
                        round(cardinality, 2) if cardinality is not None else None
                    )
                except:
                    cardinality = None
                logical_plan_graph.edge(
                    str(self.execution_nodes[node_id].parents[parent]),
                    str(node_id),
    ...
    def __push_filter__(self, node_id):
        ...
        # you have exactly one target
            ...
            if issubclass(type(node), SourceNode):
                ...
                if (
                    type(node) == InputDiskParquetNode
                    or type(node) == InputS3ParquetNode
                    or type(node) == InputS3IcebergNode
                ):
                    # try to extract pyarrow compatible predicates to push down to reader.
                    filters, remaining_predicate = sql_utils.parquet_condition_decomp(
                        predicate
                    )
                    if len(filters) > 0:
                        node.predicate = filters
                        node.targets[target_id].predicate = optimizer.simplify.simplify(
                            remaining_predicate
                        )
                    return
                else:
                ...
                # @palaska: Filter node only has 1 parent (input)
                parent_id = node.parents[0]
                parent = self.execution_nodes[parent_id]

                if target_id in parent.targets:
                    raise Exception(
                        "node with two targets detected, we should never have let the user created this graph!"
                    )

                parent.targets[target_id] = copy.deepcopy(targets[target_id])
            ...
            # we assume the predicate is in CNF format. We will go through all the conjuncts and determine which parent we can push each conjunct to
                ...
                if optimizer.simplify.simplify(predicate) == sqlglot.exp.TRUE:
                    ...
                    for conjunct in conjuncts:
                        ...
                        columns = set(
                            i.name
                            for i in conjunct.find_all(sqlglot.expressions.Column)
                        )
                        conjunct_schema_mappings = {
                            col: node.schema_mapping[col] for col in columns
                        }

                        # each value here will be a dict of parent ids that have this column and what this column is called in the parent
                        conjunct_parents = {
                            col: set(conjunct_schema_mappings[col].keys())
                            for col in columns
                        }
                        ...
                        if (
                            all(
                                [
                                    parents == conjunct_parents[col]
                                    for col in conjunct_parents
                                ]
                            )
                            and -1 not in parents
                        ):
                            for parent in parents:
                                copied_conjunct = copy.deepcopy(conjunct)
                                rename_dict = {
                                    col: node.schema_mapping[col][parent]
                                    for col in columns
                                    if col != node.schema_mapping[col][parent]
                                }
                                for identifier in copied_conjunct.find_all(
                                    sqlglot.exp.Identifier
                                ):
                                    if identifier.name in rename_dict:
                                        identifier.replace(
                                            sqlglot.exp.to_identifier(
    ...
    def __early_projection__(self, node_id):
        ...
        if issubclass(type(node), SourceNode):
            ...
            if (
                type(node) == InputDiskParquetNode
                or type(node) == InputS3ParquetNode
                or type(node) == InputDiskCSVNode
                or type(node) == InputS3CSVNode
                or type(node) == InputS3IcebergNode
            ):
                projection = set()
        ...
        # you are the one that triggered execution, you must be a SinkNode!
            ...
            if issubclass(type(node), ProjectionNode):
                ...
                for target_id in targets:
                    ...
                    assert set(target.projection).issubset(set(node.projection))
                    ...
                    parent.targets[target_id] = TargetInfo(
                        target.partitioner,
                        parent.targets[node_id].predicate,
                        target.projection,
                        target.batch_funcs,
                    )
                    ...
                    for key in self.execution_nodes[target_id].parents:
                        if self.execution_nodes[target_id].parents[key] == node_id:
                            self.execution_nodes[target_id].parents[key] = node.parents[
                                0
                            ]
                            success = True
                            break
                ...
                pushable_projections = projection.union(predicate_required_columns)
                pushed_projections = {}
                ...
                for parent_idx in self.execution_nodes[node_id].required_columns:
                    ...
                    pushed_projections[parent_id] = self.execution_nodes[
                        node_id
                    ].required_columns[parent_idx]
                ...
                for parent_idx in node.parents:
                    ...
                    if parent_id in pushed_projections:
                        ...
                        parent.targets[node_id].projection = pushed_projections[
                            parent_id
                        ]
    ...
    def __push_ann__(self):
        ...
        for node_id in self.execution_nodes:
            if issubclass(
                type(self.execution_nodes[node_id]), NearestNeighborFilterNode
            ):
        ...
        for node_id in ann_nodes:
            ...
            while True:
                if issubclass(
                    type(self.execution_nodes[curr_node_id]), SourceNode
                ) or issubclass(type(self.execution_nodes[curr_node_id]), FilterNode):
                    break
                else:
                    ...
                    assert (
                        len(interested) == 1
                    ), "vector column must only originate from one source currently, i.e. unions don't work. Please raise Github issue."
                    my_parent = list(interested.keys())[0]
                    if my_parent == -1:
                        break
                    else:
                        curr_node_id = self.execution_nodes[curr_node_id].parents[
                            my_parent
                        ]
            ...
            if issubclass(type(self.execution_nodes[curr_node_id]), InputLanceNode):
                ...
                lance_node.set_probe_df(
                    copy.deepcopy(node.probe_df), node.probe_vector_col, node.k
                )
            ...
            if issubclass(type(node), MapNode):
                ...
                for target_id in targets:
                    ...
                    if target.predicate == sqlglot.exp.TRUE:
                        ...
                        def func(x):
                            return x.filter(sql_utils.evaluate(target.predicate))
        ...
        # you are the one that triggered execution, you must be a SinkNode!
            ...
            if issubclass(type(node), JoinNode):
                ...
                if len(targets) > 1 or not all(
                    [join_spec[0] == "inner" for join_spec in node.join_specs]
                ):
                    for parent in parents:
                ...
                while True:
                    ...
                    for parent in parents:
                        ...
                        if (
                            issubclass(type(parent_node), JoinNode)
                            and len(parent_node.targets) == 1
                            and len(parent_node.targets[node_id].batch_funcs) == 0
                            and all(
                                [
                                    join_spec[0] == "inner"
                                    for join_spec in parent_node.join_specs
                                ]
                            )
                        ):
                            not_done = True
                            ...
                            for key in parent_node.parents:
                                ...
                                new_join_spec[new_key] = parent_node.join_specs[0][1][
                                    key
                                ]
                                # now we have to remove the parent node_id from the parent's parents' targets and replace it with this node's id
                                self.execution_nodes[parent_node.parents[key]].targets[
                                    node_id
                                ] = self.execution_nodes[
                                    parent_node.parents[key]
                                ].targets[
                                    parents[parent]
                                ]
                                del self.execution_nodes[
                                    parent_node.parents[key]
                                ].targets[parents[parent]]
                            ...
                            if parent_node.targets[node_id].predicate is not None:
                                if node.targets[target_id].predicate is None:
                                    node.targets[target_id].predicate = (
                                        parent_node.targets[node_id].predicate
                                    )
                                else:
                                    node.targets[target_id].predicate = (
                    ...
                    for join_spec in old_join_specs:
                        ...
                        for key in join_spec[1]:
                            ...
                            if type(new_key) == list:
                                ...
                                if (
                                    join_key
                                    in self.execution_nodes[new_parents[new_key[0]]]
                                    .targets[node_id]
                                    .projection
                                ):
                                    new_key = new_key[0]
    ...
    def __propagate_cardinality__(self, node_id):
        ...
        if issubclass(type(node), SourceNode):
            ...
            # @palaska: below code is only valid for BroadcastJoinNode and NearestNeighborFilterNode
            parent_cardinalites = {
                parent: self.execution_nodes[parents[parent]].cardinality[node_id]
                for parent in parents
            }
    ...
    # @palaska:
    #  - root of the tree is stage 0 (SinkNode)
    #  - assuming non-blocking execution, stage number propagates until a JoinNode
    #  - JoinNode has 2 parents (inputs), we propagate the stage on one side while we decrement the stage number on the other side
    #  - Only join nodes can generate new stages
    def __determine_stages__(self, node_id: NodeId):
            ...
            if issubclass(type(node), JoinNode):
                ...
                if all([join_spec[0] == "inner" for join_spec in node.join_specs]):
                    ...
                    if not self.exec_config["blocking"]:
                        ...
                        self.execution_nodes[parents[probe]].assign_stage(
                            node.stage - 1
                        )
                    for parent in parents:
                        if parent != probe:
                            if not self.exec_config["blocking"]:
                                self.execution_nodes[parents[parent]].assign_stage(
                                    node.stage - 1
                                )
                            else:
                                self.execution_nodes[parents[parent]].assign_stage(
                    ...
                    while len(old_join_specs) > 0:
                        ...
                        candidates = {
                            index: list(
                                set(node.join_specs[index][1].keys()).difference(
                                    existing_tables
                                )
                            )
                            for index in old_join_specs
                            if len(
                                set(node.join_specs[index][1].keys()).intersection(
                                    existing_tables
                                )
                            )
                            > 0
                        ...
                        assert all(
                            [len(candidates[index]) == 1 for index in candidates]
                        ), "There is a duplicate join condition, blowing up"
                        candidates = {
                            index: candidates[index][0] for index in candidates
                        }
                        assert (
                            len(candidates) > 0
                        ), "discontiguous join conditions. Blowing up"
                        candidate = min(
                            candidates,
                            key=lambda index: estimated_cardinality[candidates[index]],
                        )
                        other_table = candidates[candidate]
                        intersected_table = list(
                            set(node.join_specs[candidate][1].keys()).intersection(
                                existing_tables
                            )
                        )[0]
                        ...
                        new_join_spec = (
                            join_spec[0],
                            [
                                (intersected_table, join_spec[1][intersected_table]),
                                (other_table, join_spec[1][other_table]),
                            ],
                        )
                        new_join_specs.append(new_join_spec)
                        old_join_specs.remove(candidate)
                        existing_tables = existing_tables.union(
                            set(join_spec[1].keys())
                        )
                    ...
                    assert (
                        len(node.join_specs) == 1
                    ), "There are multiple joins and some of them are not inner joins, blowing up"
                    ...
                    node.join_specs = [
                        (join_spec[0], [(0, join_spec[1][0]), (1, join_spec[1][1])])
                    ]
                    if not self.exec_config["blocking"]:
...
class DataSet(IDataSet):
    def __init__(
        self, quokka_context: QuokkaContext, schema: dict, source_node_id: NodeId
    ) -> None:

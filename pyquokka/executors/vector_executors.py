...
class DFProbeDataStreamNNExecutor2(Executor):
    ...
    def execute(self, batches: list[pa.Table], stream_id, executor_id):
    ...
    def done(self, executor_id):
        ...
        normalized_vectors = normalized_vectors.reshape(
            -1, len(self.query_df), self.k, vector_dim
        )
        N = normalized_vectors.shape[0]
        normalized_vectors = normalized_vectors.transpose(1, 0, 2, 3).reshape(
            len(self.query_df), N * self.k, vector_dim
        )
        ...
        with threadpool_limits(limits=8, user_api="blas"):
            distances = np.matmul(
                normalized_vectors, query_vecs.transpose(0, 2, 1)
            ).squeeze()
        ...
        for i in range(len(indices)):
            ...
            new_indices = (
                index_group * len(self.query_df) * self.k + i * self.k + index_in_group
            )
        ...
        return pa.Table.from_arrays(
            matched_vectors.columns + matched_queries.columns,
            names=matched_vectors.column_names + matched_queries.column_names,
        )

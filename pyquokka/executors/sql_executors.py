...
class ConcatThenSQLExecutor(Executor):
    ...
    def execute(self, batches, stream_id, executor_id):
        ...
        self.state = (
            batch if self.state is None else pa.concat_tables([self.state, batch])
        )
...
class SuperFastSortExecutor(Executor):
    def __init__(
        self,
        key,
        record_batch_rows=100000,
        output_batch_rows=1000000,
        file_prefix="mergesort",
    ) -> None:
        self.key = key
    ...
    def execute(self, batches, stream_id, executor_id):
        ...
        start = time.time()
        new_in_mem_state = polars.from_dict(
            {
                "values": sorted_batch[self.key],
                "file_no": np.ones(len(batch), dtype=np.int32) * self.fileno,
            }
        )
    ...
    def done(self, executor_id):
        ...
        number_of_batches_in_source = {
            source: sources[source].num_record_batches for source in sources
        }
        cached_batches = {
            i: polars.from_arrow(pa.Table.from_batches([sources[i].get_batch(0)]))
            for i in sources
        }
        ...
        for k in range(0, len(self.in_mem_state), self.output_batch_rows):
            ...
            for i in range(len(file_requirements)):
                ...
                while desired_length > len(cached_batches[source]):
                    if (
                        current_number_for_source[source]
                        == number_of_batches_in_source[source]
                    ):
                        raise Exception
                    else:
                        cached_batches[source].vstack(
                            polars.from_arrow(
                                pa.Table.from_batches(
                                    [
                                        sources[source].get_batch(
                                            current_number_for_source[source]
                                        )
                                    ]
                                )
...
class OutputExecutor(Executor):
    def __init__(
        self, filepath, format, prefix="part", region="local", row_group_size=5000000
    ) -> None:
        self.num = 0
    ...
    def upload_write_batch(self, write_batch, executor_id):
        ...
        if self.executor is None:
            ...
            self.fs = (
                ...
                else S3FileSystem(region=self.region)
            )
        ...
        if self.format == "csv":
            for i, (col_name, type_) in enumerate(
                zip(write_batch.schema.names, write_batch.schema.types)
            ):
                if pa.types.is_decimal(type_):
                    write_batch = write_batch.set_column(
                        i,
                        col_name,
                        compute.cast(write_batch.column(col_name), pa.float64()),
                    )
            ...
            if self.format == "parquet":
                futures.append(
                    self.executor.submit(
                        upload_parquet, current_batch, basename_template
                    )
                )
            else:
                futures.append(
                    self.executor.submit(upload_csv, current_batch, basename_template)
                )
    ...
    def execute(self, batches, stream_id, executor_id):
        ...
        write_batch = (
            pa.concat_tables(self.my_batches[:full_batches_to_take])
            if full_batches_to_take > 0
            else None
        )
        ...
        return_df = polars.from_dict(
            {
                "filename": [
                    ...
                    for i in range(len(write_batch) // self.row_group_size)
                ]
            }
        )
        return return_df
        ...
        return_df = polars.from_dict(
            {
                "filename": [
                    ...
                    for i in range((len(df) - 1) // self.row_group_size + 1)
                ]
            }
        )
        return return_df
...
class BuildProbeJoinExecutor(Executor):
    ...
    def __init__(
        self, on=None, left_on=None, right_on=None, how="inner", key_to_keep="left"
    ):
    ...
    def execute(self, batches, stream_id: SourceDataStreamIndex, executor_id):
        ...
        if stream_id == 1:
            ...
            self.state = (
                batch if self.state is None else self.state.vstack(batch, in_place=True)
            )
        ...
        # probe
            ...
            self.phase = "probe"
            result = batch.join(
                self.state, left_on=self.left_on, right_on=self.right_on, how=self.how
            )
    ...
    def execute(self, batches, stream_id, executor_id):
        ...
        # probe
            ...
            if self.how == "semi":
                ...
                result = polars.from_arrow(
                    con.execute(
                        "select * from d semi join u1 on d.l_orderkey = u1.l_orderkey and d.l_suppkey <> u1.l_suppkey"
                    ).arrow()
                )
                ...
                result = polars.from_arrow(
                    con.execute(
                        "select * from d anti join u2 on d.l_orderkey = u2.l_orderkey and d.l_suppkey <> u2.l_suppkey"
                    ).arrow()
                )
...
class DiskBuildProbeJoinExecutor(Executor):
    ...
    def execute(self, batches, stream_id, executor_id):
        ...
        if stream_id == 1:
            ...
            batch.write_parquet(
                f"{self.spill_dir}/build_{self.prefix}_{executor_id}_{self.count}.parquet"
            )
            self.count += 1
        ...
        # probe
            ...
            for i in range(self.count):
                ...
                results.append(
                    batch.lazy().join(
                        polars.scan_parquet(
                            f"{self.spill_dir}/build_{self.prefix}_{executor_id}_{i}.parquet"
                        ),
                        left_on=self.left_on,
                        right_on=self.right_on,
                        how=self.how,
